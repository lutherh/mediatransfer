...
console.log('⬆️ Uploading Takeout archives to Scaleway (incremental, one archive at a time)...');
...
const result = await runTakeoutIncremental(
  ...
  provider,
  {
    ...
    deleteArchiveAfterUpload: deleteArchives,
    moveArchiveAfterUpload: moveArchives,
    completedArchiveDir,
    progressIntervalMs,
    onArchiveStart(archiveName, index, total) {
      console.log(`\n📦 [${index}/${total}] Processing archive: ${archiveName}`);
    },
