...
export type DuplicateGroup = {
  ...
  /**
   * True when the group contains keys from both the `immich/` and `s3transfers/`
   * namespaces. The algorithm keeps the `immich/` copy so the `s3transfers/` copy
...
export class ScalewayCatalogService implements CatalogService {
  ...
  async moveObject(encodedKey: string, newDatePrefix: string): Promise<{ from: string; to: string }> {
    ...
    const topDir = oldSegments[0] === S3TRANSFERS_PREFIX ? `${S3TRANSFERS_PREFIX}/` : '';
 ...
 * Keys after strip are `s3transfers/YYYY/MM/DD/filename`, so when the user
 * types a bare date like "2020/02" we auto-prepend "s3transfers/".
 */
function normalizeSearchPrefix(query?: string): string | undefined {
  if (!query) return undefined;
  // Already includes the s3transfers/ prefix — pass through
  if (/^s3transfers\//i.test(query)) return query;
  // Starts with a date-like digit sequence (YYYY…) — auto-prepend s3transfers/
  if (/^\d/.test(query)) return `s3transfers/${query}`;
  return query;
}
 ...
 *     will never score below a `s3transfers/` path (+8 max).
 *  2. Dated transfers path (`s3transfers/YYYY/MM/DD/`)                 (+8)
 *  3. All other paths                                                 (0)
...
export function scoreKeyForKeep(key: string): number {
  ...
  if (key.startsWith('immich/library/')) {
    ...
    score += 30;
  } else if (/^s3transfers\/((?:19|20)\d{2})\/\d{2}\/\d{2}\//.test(key)) {
    // Dated transfers path like s3transfers/2020/03/15/file.jpg
    score += 8;
  }
...
export function buildDuplicateGroups(
  ...
  for (const [fingerprint, items] of map) {
    ...
    const hasTransfers = allKeys.some((k) => k.startsWith(`${S3TRANSFERS_PREFIX}/`));
